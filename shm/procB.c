...
union semun {
    int val;
    struct semid_ds *buf; /* representa um grupo de semáforos */
    ushort *array;
};
...
{	
	...
	for(;;){
        ...
        printf("[leitura]: %s", memoria_compartilhada);
        
        /* Verifica condição de saída */
        if(!strcmp(memoria_compartilhada, "sair\n"))
        {
        	puts("Saindo!");
        	break;
        }
...
int initsem()
{
    ...
    */
    ...
    {   
        printf("Criando um grupo de semaforos[id=%i]<enter> para continuar...\n", grupo_semaforo_id); 
        getchar();
    ...
    { 
    	...
    	printf("Conectando a um grupo de semaforos[id=%i] <enter> para continuar...\n", grupo_semaforo_id); 
        getchar();
